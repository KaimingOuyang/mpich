...
int MPIDI_GPU_mpi_init_hook(int rank, int size, int *tag_bits)
{
    ...
    for (int i = 0; i < MPIR_Process.local_size; ++i) {
        ...
        if (i == MPIR_Process.local_rank) {
            for (int j = 0; j < node_max_dev_id; ++j) {
                MPIDI_gpu_dev_id_t *tmp = NULL;
                HASH_FIND_INT(MPIDI_gpu_global.global_to_local_map, &j, tmp);
                if (tmp)
                    MPIDI_gpu_global.visible_dev_global_id[i][j] = 1;
                else
                    MPIDI_gpu_global.visible_dev_global_id[i][j] = 0;
            }
            MPIDU_Init_shm_put(MPIDI_gpu_global.visible_dev_global_id[i],
                               sizeof(int) * node_max_dev_id);
        }
    }
    MPIDU_Init_shm_barrier();
    ...
    for (int i = 0; i < MPIR_Process.local_size; ++i)
        MPIDU_Init_shm_get(i, sizeof(int) * node_max_dev_id,
                           MPIDI_gpu_global.visible_dev_global_id[i]);
    MPIDU_Init_shm_barrier();
