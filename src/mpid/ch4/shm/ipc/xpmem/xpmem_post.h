...
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_get_ipc_attr(const void *vaddr,
                                                      MPIDI_IPCI_ipc_attr_t * ipc_attr)
{
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_XPMEM_IPC_ATTR_CREATE);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_XPMEM_IPC_ATTR_CREATE);

    memset(&ipc_attr->ipc_handle, 0, sizeof(MPIDI_IPCI_ipc_handle_t));

#ifdef MPIDI_CH4_SHM_ENABLE_XPMEM
    ipc_attr->ipc_type = MPIDI_IPCI_TYPE__XPMEM;
    ipc_attr->ipc_handle.xpmem = (uint64_t) vaddr;
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_XPMEM_IPC_ATTR_CREATE);
    return MPI_SUCCESS;
}
