...
#undef FUNCNAME
#define FUNCNAME MPIDI_XPMEM_lmt_coop_isend
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_XPMEM_lmt_coop_isend(const void *buf, MPI_Aint count,
                                                        MPI_Datatype datatype, int rank, int tag,
                                                        MPIR_Comm * comm, int context_offset,
                                                        MPIDI_av_entry_t * addr, int packet_type,
                                                        MPIR_Request ** request)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_XPMEM_LMT_COOP_ISEND);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_XPMEM_LMT_COOP_ISEND);
    ...
    mpi_errno = MPIDI_SHM_do_ctrl_send(rank, comm, packet_type, &ctrl_hdr);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_XPMEM_LMT_COOP_ISEND);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
